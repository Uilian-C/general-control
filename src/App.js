...
// --- NOVO COMPONENTE DE CABEÇALHO PADRONIZADO ---
const ViewHeader = ({ icon, title, description, actions }) => (
    <Card>
...
// ... (Restante dos componentes de Modal, TaskModal, CyclesModal, etc. permanecem os mesmos)
// NOTE: O restante do código de componentes como modais, forms, etc. não foi alterado pois o foco era layout e exportação.
// O código completo original será colado abaixo, mas as principais mudanças estão aqui e no final do arquivo.
...
// --- VISÃO WORKSPACE (ROADMAP) ATUALIZADA ---
const WorkspaceView = ({ tasks, cycles, onTaskClick, filters, setFilters, zoomLevel, setZoomLevel, viewStartDate, setViewStartDate, onOpenTaskModal, onOpenCyclesModal }) => {
    ...
    return (
        <div className="space-y-6" ref={viewRef}>
            <ViewHeader
                icon={<Layers className="mr-3 text-indigo-600" />}
                title="Roadmap"
                description="Planeje e visualize suas iniciativas ao longo do tempo."
                actions={
                    <>
                        <Button
            ...
            <Card>
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 no-export">
                        <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-wrap justify-start items-center gap-4 border-t pt-4 mt-2 no-export">
...
const ExecutiveView = ({ tasks, okrs, onSaveOkr }) => {
    ...
    return (
        <div className="space-y-6" ref={viewRef}>
            <ViewHeader
                icon={<Briefcase className="mr-3 text-indigo-600" />}
                title="Painel Executivo"
                description="Visão consolidada do progresso, metas e riscos."
                actions={
                    <Button
...
const OkrView = ({ okrs, tasks, onSave, onDelete }) => {
    ...
    return (
        <div className="space-y-6" ref={viewRef}>
            ...
            <ViewHeader
                icon={<Target className="mr-3 text-indigo-600" />}
                title="Objetivos e Resultados-Chave"
                description="Defina e acompanhe as metas que impulsionam seu roadmap."
                actions={
                    <>
...
// --- VISÃO DE ATIVIDADES (KANBAN) ATUALIZADA ---
const TasksView = ({ tasks, onTaskClick, filters, setFilters, onOpenTaskModal }) => {
    ...
    return (
        <div className="space-y-6" ref={viewRef}>
             <ViewHeader
                icon={<ListTodo className="mr-3 text-indigo-600" />}
                title="Quadro de Atividades"
                description="Visualize e gerencie o fluxo de trabalho do dia a dia."
                actions={
                    <>
                        <Button
                        ...
                        <Button onClick={() => onOpenTaskModal(null)} variant="primary">
                            <Plus size={16} /> Nova Tarefa
                        </Button>
                    </>
                }
            ...
            <div className="flex gap-6 overflow-x-auto pb-4">
                {Object.keys(STATUSES).map(status => (
                    <div key={status} className="flex-shrink-0 w-96 bg-gray-100 rounded-xl">
                        ...
                        <div className="p-4 space-y-4 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 400px)' }}>
            ...
            <div className="max-w-screen-2xl mx-auto p-4 sm:p-6 lg:p-8">
                <header className="mb-6 no-export">
                    ...
                    <div className="mt-6 border-b border-gray-200">
                        <div className="inline-flex items-center -mb-px bg-transparent rounded-lg space-x-1">
                            <button onClick={() => setView('tasks')} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 border-b-2 ${view === 'tasks' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><ListTodo size={16} /> Atividades</button>
                            <button onClick={() => setView('workspace')} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 border-b-2 ${view === 'workspace' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><Layers size={16} /> Roadmap</button>
                            <button onClick={() => setView('okr')} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 border-b-2 ${view === 'okr' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><Target size={16} /> OKRs</button>
                            <button onClick={() => setView('executive')} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 border-b-2 ${view === 'executive' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><Briefcase size={16} /> Painel Executivo</button>
                        </div>
                    </div>
                </header>
