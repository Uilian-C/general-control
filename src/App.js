...
import { Target, Flag, Plus, Trash2, X, Layers, Briefcase, Edit, Settings, ChevronLeft, ChevronRight, Tag, Palette, AlertCircle, TrendingUp, History, ChevronsUpDown, Download, Paperclip, Calendar, ListTodo, ZoomIn, ZoomOut, Circle } from 'lucide-react';
...
const TaskModal = ({ isOpen, onClose, task, tasks, okrs, onSave, onDeleteRequest }) => {
    ...
    const cleanInputClass = "w-full p-2 bg-transparent border-b-2 border-gray-300 focus:border-indigo-500 focus:outline-none focus:ring-0 transition-colors";
    const cleanSelectClass = `${cleanInputClass} appearance-none`;
    const sectionClass = "p-4 bg-gray-50/50 rounded-lg";
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={task?.humanId ? `Editar Tarefa [${task.humanId}]` : "Nova Tarefa"} size="2xl">
            <div className="space-y-6 text-gray-700">
                ...
                <section className={sectionClass}>
                    ...
                    <div className="space-y-4">
                        <input type="text" name="title" value={currentTask.title || ''} onChange={handleChange} placeholder="Título da Tarefa" className={`${cleanInputClass} text-xl font-bold`} />
                ...
                <section className={sectionClass}>
                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Planejamento</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                        <div>
                ...
                <section className={sectionClass}>
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">Subtarefas</h3>
                    <div className="space-y-1 max-h-48 overflow-y-auto pr-2">
                        {currentTask.subtasks && currentTask.subtasks.map((sub, index) => (
                            <div key={sub.id} className="flex items-center gap-3 py-1 border-b border-gray-200 last:border-b-0">
                ...
                <section className={sectionClass}>
                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Conexões</h3>
                    <div>
                ...
                <footer className="flex justify-between items-center space-x-4 pt-5 mt-5 border-t border-gray-200">
                    <div>
                                            ...
                                            {groupedTasks[group].map((task, taskIndex) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        title={task.title}
                                                    >
                                                        <div className={`h-full w-full rounded-lg flex items-center px-2 overflow-hidden relative shadow-md group-hover:shadow-lg group-hover:scale-[1.02] transition-all duration-200 bg-gradient-to-r from-gray-400 to-gray-500`}>
                                                            <div className="absolute top-0 left-0 h-full bg-black/20 rounded-lg" style={{ width: `${subtaskProgress}%` }}></div>
                                                            <div className="absolute top-0 left-0 h-full w-full bg-gradient-to-r from-indigo-500 to-sky-500 opacity-80" style={{ width: `${subtaskProgress}%` }}></div>
                                                            
                                                            <div className="relative z-10 flex items-center gap-2 w-full truncate">
                                                              <Circle size={10} className={`${STATUSES[task.status]?.iconColor} fill-current flex-shrink-0`} />
    ...
    const handleSaveTask = async (taskData) => {
        ...
        if (taskData.id) {
            ...
            const newTask = {
                ...
                humanId: `T-${Math.random().toString(16).slice(2, 6).toUpperCase()}`
            };
