...
// Configuração do Firebase restaurada do arquivo original para corrigir o erro de API Key.
...
export default function App() {
    ...
    const filteredTasks = useMemo(() => {
        ...
        if (filters.priority !== 'Todos') {
            sortedTasks = sortedTasks.filter(task => task.priority === filters.priority);
        }
        if (filters.status !== 'Todos') {
            sortedTasks = sortedTasks.filter(task => task.status === filters.status);
        }
